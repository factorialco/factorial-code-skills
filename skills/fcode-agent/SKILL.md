...
(every form is public), a button inside the Factorial UI (`fcode-ui-triggers`)
or any other Factorial Action entry point (`fcode-factorial-actions`), or
exposing the process as an MCP tool — and pushing to cloud when ready. Pushing never affects consumers pinned
